...
<div
	class="flex overflow-x-auto gap-6 snap-x snap-mandatory before:shrink-0 before:w-[33vw] after:shrink-0 after:w-[33vw]"
>
	{#each data as movie, index}
		<div id={getItemId(index)} class="shrink-0 snap-center">
			<img
	...
	<div class="absolute flex justify-between transform -translate-y-1/2 left-5 right-5 top-1/2">
		<a on:click={decrease} href={`#${getItemId(prev)}`} class="btn btn-circle scroll-smooth"> ❮ </a>
		<a on:click={increase} href={`#${getItemId(next)}`} class="btn btn-circle scroll-smooth"> ❯ </a>
	</div>
</div>
